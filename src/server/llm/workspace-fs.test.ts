...
// 8B 급 로컬 모델이 end_line 으로부터 next offset 을 산술 계산하지 못하는 회귀를
// 우회하기 위해 명시적 next_offset 필드를 노출한다. has_more=true 면 다음 청크의
// 시작 줄(=end_line) 이고, has_more=false 면 null.
test('readFileChunk: next_offset 은 has_more 와 동기화된다', async () => {
  const ws = makeWorkspace();
  try {
    const lines = Array.from({ length: 50 }, (_, i) => `line ${i + 1}`);
    writeFileSync(path.join(ws, 'mid.txt'), lines.join('\n') + '\n', 'utf8');
    const head = await readFileChunk(ws, 'mid.txt', 0, 20);
    assert.equal(head.has_more, true);
    assert.equal(head.next_offset, 20, 'has_more=true 면 next_offset=end_line 이어야 함');
    const mid = await readFileChunk(ws, 'mid.txt', head.next_offset!, 20);
    assert.equal(mid.start_line, 20);
    assert.equal(mid.has_more, true);
    assert.equal(mid.next_offset, 40);
    const tail = await readFileChunk(ws, 'mid.txt', mid.next_offset!, 20);
    assert.equal(tail.has_more, false);
    assert.equal(tail.next_offset, null, 'has_more=false 면 next_offset=null');
  } finally { cleanup(ws); }
});
...
// 로컬 LLM 토큰 폭발 회귀 — 200+ 파일 프로젝트에서 listFiles 가 평탄 전체 트리를
// 한 번에 반환하면 모델 컨텍스트가 터진다. maxDepth 인자로 첫 호출을 좁힐 수 있어야
// 하고, 깊이별로 하위 노출 여부가 정확해야 한다.
test('listFiles: maxDepth=1 은 dir 직속만 노출하고 더 깊이 안 들어간다', async () => {
  const ws = makeWorkspace();
  try {
    mkdirSync(path.join(ws, 'src/components/forms'), { recursive: true });
    mkdirSync(path.join(ws, 'docs'), { recursive: true });
    writeFileSync(path.join(ws, 'README.md'), '# root');
    writeFileSync(path.join(ws, 'src/index.ts'), 'export {}');
    writeFileSync(path.join(ws, 'src/components/Button.tsx'), 'x');
    writeFileSync(path.join(ws, 'src/components/forms/Input.tsx'), 'x');
    writeFileSync(path.join(ws, 'docs/guide.md'), 'x');
    const entries = await listFiles(ws, '.', undefined, 1);
    const paths = entries.map(e => e.path).sort();
    // 직속 디렉터리 + 직속 파일만 보여야 한다.
    assert.deepEqual(paths, ['README.md', 'docs', 'src'].sort());
    // 디렉터리는 'dir' 타입.
    assert.ok(entries.find(e => e.path === 'src')?.type === 'dir');
    assert.ok(entries.find(e => e.path === 'docs')?.type === 'dir');
    // 'src/index.ts' 같은 한 단계 아래는 절대 노출되면 안 된다.
    assert.ok(!paths.some(p => p.includes('/')), `flat 직속만 — 발견: ${paths.filter(p => p.includes('/')).join(', ')}`);
  } finally { cleanup(ws); }
});

test('listFiles: maxDepth=2 는 한 단계 더 들어가고 그 너머는 안 보인다', async () => {
  const ws = makeWorkspace();
  try {
    mkdirSync(path.join(ws, 'src/components/forms'), { recursive: true });
    writeFileSync(path.join(ws, 'src/index.ts'), 'export {}');
    writeFileSync(path.join(ws, 'src/components/Button.tsx'), 'x');
    writeFileSync(path.join(ws, 'src/components/forms/Input.tsx'), 'x');
    const entries = await listFiles(ws, '.', undefined, 2);
    const paths = entries.map(e => e.path);
    assert.ok(paths.includes('src'), 'src/ dir');
    assert.ok(paths.includes('src/index.ts'), 'src 직속 파일');
    assert.ok(paths.includes('src/components'), 'src 직속 dir');
    // 'src/components/Button.tsx' 는 dir='.' 기준 2 depth(. → src → components → Button.tsx)
    // 인데 maxDepth=2 면 components/ 까지만 보이고 그 안의 파일은 안 보여야 한다.
    assert.ok(!paths.includes('src/components/Button.tsx'), 'maxDepth=2 너머는 숨김');
    assert.ok(!paths.includes('src/components/forms'), 'maxDepth=2 너머는 숨김');
  } finally { cleanup(ws); }
});

test('listFiles: maxDepth 미지정시 기존 동작 유지(전체 재귀)', async () => {
  const ws = makeWorkspace();
  try {
    mkdirSync(path.join(ws, 'a/b/c'), { recursive: true });
    writeFileSync(path.join(ws, 'a/b/c/deep.txt'), 'x');
    const entries = await listFiles(ws);
    const paths = entries.map(e => e.path);
    assert.ok(paths.includes('a/b/c/deep.txt'), '깊은 파일까지 노출돼야 함');
  } finally { cleanup(ws); }
});
