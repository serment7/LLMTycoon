...
//   R9. requireCitations — 근거가 비어 있는 서브쿼리 섹션은 생략되고 limitations 에 기록된다.
//   R10. deadline 필터 — ISO 날짜 이전 publishedAt 을 가진 결과는 citations 에서 제외된다.
...
// ────────────────────────────────────────────────────────────────────────────
// R6. 부분 보고서 정책 — 토큰 예산 초과 시 완료분이 details.partial 에 담긴다
// ────────────────────────────────────────────────────────────────────────────

test('R6. 토큰 예산 초과 시 details.partial.sections 에 완료분이 담긴다', async () => {
...
// ────────────────────────────────────────────────────────────────────────────
// R9. requireCitations — 근거 0 인 서브쿼리 섹션은 생략된다
// ────────────────────────────────────────────────────────────────────────────

test('R9. requireCitations=true 면 근거 0 인 서브쿼리는 섹션에서 제외되고 limitations 에 기록된다', async () => {
  // 첫 서브쿼리에만 근거 2건을 주고, 나머지는 빈 배열을 돌려준다.
  let callIdx = 0;
  const runtime: ResearchRuntime = {
    searchRunner: async () => {
      callIdx += 1;
      if (callIdx === 1) {
        return [
          makeResult({ url: 'https://openai.com/a' }),
          makeResult({ url: 'https://arxiv.org/abs/1' }),
        ];
      }
      return [];
    },
  ...
  const report = await research(
    '부분 생략',
    { depth: 1, breadth: 3, requireCitations: true },
    runtime,
  );
  // 한 섹션 이상은 완성되어야 한다(첫 서브쿼리 기여분).
  assert.ok(report.sections.length >= 1, '근거 있는 서브쿼리 섹션은 살아있어야 함');
  // 최소 한 번은 "근거가 부족" 안내가 limitations 에 추가되어야 한다.
  assert.ok(
    report.limitations.some((l) => /근거가 부족/.test(l)),
    'requireCitations 로 생략된 서브쿼리가 limitations 에 반영되어야 함',
  );
});

// ────────────────────────────────────────────────────────────────────────────
// R10. deadline 필터 — 지정 날짜 이전 publishedAt 결과는 제외
// ────────────────────────────────────────────────────────────────────────────

test('R10. options.deadline 이전에 게시된 결과는 citations 에서 제외된다', async () => {
  const runtime: ResearchRuntime = {
    searchRunner: stubRunner((sub) => [
      makeResult({
        url: `https://old.example/${encodeURIComponent(sub.question)}`,
        publishedAt: '2020-01-01',
      }),
      makeResult({
        url: `https://fresh.example/${encodeURIComponent(sub.question)}`,
        publishedAt: '2026-01-01',
      }),
      // publishedAt 이 없는 결과는 보수적으로 포함되어야 한다.
      makeResult({
        url: `https://undated.example/${encodeURIComponent(sub.question)}`,
      }),
    ]),
  };
  const report = await research(
    '기한 필터',
    { depth: 1, breadth: 2, deadline: '2024-01-01' },
    runtime,
  );
  assert.ok(report.citations.length > 0, 'citations 비어있으면 안 됨');
  for (const c of report.citations) {
    assert.ok(
      !c.url.startsWith('https://old.example'),
      `${c.url} 은 deadline 이전이므로 제외되어야 함`,
    );
  }
  // 신선한 자료는 최소 1건 포함되어야 한다.
  assert.ok(
    report.citations.some((c) => c.url.startsWith('https://fresh.example')),
    'deadline 이후 자료는 citations 에 남아야 함',
  );
});

// ────────────────────────────────────────────────────────────────────────────
// R11. 서로 다른 서브쿼리가 같은 자원을 서로 다른 추적 파라미터로 반환해도,
//       증거 매칭은 정규화된 URL 기준으로 이뤄져 섹션이 생략되지 않아야 한다.
// ────────────────────────────────────────────────────────────────────────────

test('R11. 크로스 서브쿼리 추적 파라미터 변이에도 requireCitations 섹션이 유지된다', async () => {
  // 두 서브쿼리가 동일한 "공유 자원" 을 서로 다른 utm 으로 가져온다. dedup 은
  // 공유 자원을 1건으로 합치지만, 각 서브쿼리는 자신만의 고유 자원도 1건씩
  // 돌려주므로 전체 근거는 3건이 된다. 수정 전에는 sub2 의 증거 매칭이 원본
  // URL 기준으로 이뤄져 공유 자원이 sub2 의 localEvidence 에서 누락되었고,
  // requireCitations 로는 섹션이 2개 모두 살아야 하지만 매칭 부재 시에도
  // 고유 자원으로 인해 여전히 섹션이 남아 회귀가 눈에 띄지 않았다. 따라서
  // citationNumbers 길이로 공유 자원이 반영됐는지 검사한다.
  let callIdx = 0;
  const runtime: ResearchRuntime = {
    searchRunner: async () => {
      callIdx += 1;
      if (callIdx === 1) {
        return [
          makeResult({ url: 'https://openai.com/shared?utm_source=one', title: '공유' }),
          makeResult({ url: 'https://arxiv.org/abs/sub1', title: '고유1' }),
        ];
      }
      return [
        makeResult({ url: 'https://openai.com/shared?utm_source=two', title: '공유' }),
        makeResult({ url: 'https://arxiv.org/abs/sub2', title: '고유2' }),
      ];
    },
  };
  const report = await research(
    '추적 파라미터 변이',
    { depth: 1, breadth: 2, requireCitations: true },
    runtime,
  );
  // 공유 자원은 1건으로 dedup, 고유 자원 2건 포함 → citations 3건.
  assert.equal(report.citations.length, 3);
  assert.equal(report.sections.length, 2, '두 서브쿼리 모두 섹션이 완성되어야 함');
  // 공유 자원의 citation 번호를 찾아, 두 섹션 모두 이 번호를 인용하는지 확인한다.
  const sharedCitation = report.citations.find((c) => c.url.startsWith('https://openai.com/shared'));
  assert.ok(sharedCitation, '공유 자원 citation 필요');
  for (const section of report.sections) {
    assert.ok(
      section.citationNumbers.includes(sharedCitation!.n),
      `${section.title} 섹션은 공유 자원(${sharedCitation!.n})을 인용해야 함`,
    );
  }
  assert.ok(
    report.limitations.every((l) => !/근거가 부족/.test(l)),
    '정규화 매칭이 정상이라면 근거 부족 메시지가 나오면 안 됨',
  );
});

// ────────────────────────────────────────────────────────────────────────────
// normalizeUrl 공개 계약 — dedup·증거 매칭 양쪽이 같은 기준을 쓰게 잠근다.
// ────────────────────────────────────────────────────────────────────────────

test('normalizeUrl — hash·추적 파라미터·끝 슬래시를 동등하게 제거', () => {
  assert.equal(
    normalizeUrl('https://example.com/a?utm_source=x&id=1#frag'),
  ...
  assert.equal(
    normalizeUrl('https://example.com/a?fbclid=xx'),
    'https://example.com/a',
  );
  // 파싱 실패 시 trim 된 원문 유지.
