...
test('R11. 크로스 서브쿼리 추적 파라미터 변이에도 requireCitations 섹션이 유지된다', async () => {
  // 두 서브쿼리가 동일 자원을 다른 utm 으로 반환. dedup 후 citation 은 1건만 남는다.
  ...
  const runtime: ResearchRuntime = {
    searchRunner: async () => {
      ...
      if (callIdx === 1) {
        return [
          makeResult({ url: 'https://openai.com/shared?utm_source=one', title: 'A' }),
        ];
      }
      // 이후 서브쿼리는 같은 자원을 다른 추적 파라미터로 되돌려준다.
      return [
        makeResult({ url: 'https://openai.com/shared?utm_source=two', title: 'A' }),
      ];
    },
  };
  void runners;
  ...
  // citation 은 정규화 후 1 건. 두 서브쿼리 모두 이를 근거로 삼아 섹션이 유지된다.
