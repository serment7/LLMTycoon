...
export function TokenUsageSettingsPanel({ initial, onClose, onApply, onResetToday }: Props) {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!saved) {
      setError(t('tokenUsage.settings.saveError'));
      return;
    }
    setError(null);
    onApply(next);
  ...
  return (
    <div
      role="dialog"
      aria-label={t('tokenUsage.settings.ariaLabel')}
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--token-usage-axis-input-fg)' }}>
          {t('tokenUsage.settings.title')}
        </span>
        <button
          type="button"
          onClick={onClose}
          aria-label={t('tokenUsage.settings.close')}
      ...
      <form onSubmit={handleSave} className="space-y-3" noValidate>
        <fieldset className="space-y-1" style={{ borderLeft: `3px solid var(--token-usage-caution-border)`, paddingLeft: 8 }}>
          <legend className="text-[10px] uppercase" style={{ color: 'var(--token-usage-caution-fg)' }}>
            {t('tokenUsage.settings.cautionLegend')}
          </legend>
          <label className="flex items-center justify-between gap-2">
            <span>{t('tokenUsage.settings.tokensLabel')}</span>
            <input
              type="number"
              min={1}
              inputMode="numeric"
              ...
              placeholder={t('tokenUsage.settings.tokensCautionPlaceholder')}
          ...
          <label className="flex items-center justify-between gap-2">
            <span>{t('tokenUsage.settings.usdLabel')}</span>
            <input
              type="number"
              min={0.01}
              step={0.01}
              ...
              placeholder={t('tokenUsage.settings.usdCautionPlaceholder')}
        ...
        <fieldset className="space-y-1" style={{ borderLeft: `3px solid var(--token-usage-warning-border)`, paddingLeft: 8 }}>
          <legend className="text-[10px] uppercase" style={{ color: 'var(--token-usage-warning-fg)' }}>
            {t('tokenUsage.settings.warningLegend')}
          </legend>
          <label className="flex items-center justify-between gap-2">
            <span>{t('tokenUsage.settings.tokensLabel')}</span>
            <input
              ...
              placeholder={t('tokenUsage.settings.tokensWarningPlaceholder')}
          ...
          <label className="flex items-center justify-between gap-2">
            ...
            <input
              ...
              placeholder={t('tokenUsage.settings.usdWarningPlaceholder')}
        ...
        >
          <legend className="text-[10px] uppercase" style={{ color: 'var(--token-usage-axis-input-fg)' }}>
            {t('tokenUsage.settings.exportLegend')}
          </legend>
          <div role="radiogroup" aria-label={t('tokenUsage.settings.exportRangeAria')} className="flex items-center gap-3 text-[10px]">
            {(['today', 'week', 'all'] as UsageRange[]).map(r => (
              <label key={r} className="flex items-center gap-1 cursor-pointer">
                ...
                />
                <span>{r === 'today' ? t('tokenUsage.settings.rangeToday') : r === 'week' ? t('tokenUsage.settings.rangeWeek') : t('tokenUsage.settings.rangeAll')}</span>
              </label>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-white/10">
          ...
          >
            {t('tokenUsage.settings.clearAll')}
          </button>
          <div className="flex items-center gap-2">
            {onResetToday && (
              ...
              >
                <RotateCcw size={10} /> {t('tokenUsage.settings.resetToday')}
              </button>
            )}
            <button
              type="submit"
            ...
            >
              <Save size={10} /> {t('tokenUsage.settings.save')}
            </button>
          </div>
        </div>
      </form>
