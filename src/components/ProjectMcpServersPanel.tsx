...
//   · env 는 key=value 한 줄씩 입력 → 파서가 공백 · NUL · 유효하지 않은 키를 차단
...
export function ProjectMcpServersPanel({ projectId, store, onLog }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      await api.add(input);
      setName(''); setCommand(''); setArgsInput(''); setEnvInput('');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div className="space-y-2">
          ...
          <input
            ...
            maxLength={48}
          />
          <label className="block text-[10px] uppercase tracking-wider text-white/70 pt-2">command</label>
          <input
            value={command}
        ...
        <div className="space-y-2">
          <label className="block text-[10px] uppercase tracking-wider text-white/70">args (공백 구분)</label>
          <input
            value={argsInput}
            ...
            className={`w-full bg-black/30 border-2 border-[var(--pixel-border)] px-3 py-2 text-[12px] text-white font-mono ${focusRing}`}
          />
          <label className="block text-[10px] uppercase tracking-wider text-white/70 pt-2">env (한 줄에 KEY=VALUE)</label>
          <textarea
            value={envInput}
        ...
        {items.map((rec) => (
          <article key={rec.id} className="bg-[#0f3460] border-2 border-[var(--pixel-border)] p-3 flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <h4 className="text-sm font-bold text-[var(--pixel-accent)] truncate font-mono">{rec.name}</h4>
