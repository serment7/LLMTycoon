...
test('TC-LPT-02b: unseen 카운트가 100 이상이면 배지 텍스트는 "99+" 로 축약되고 aria-label 은 실제 숫자 유지', () => {
  const { container, getByTestId } = render(<Harness />);
  const addLogBtn = getByTestId('harness-add-log');
  // 지시 탭 유지 상태에서 로그 150건 폭주.
  act(() => {
    for (let i = 0; i < 150; i++) fireEvent.click(addLogBtn);
  });
  const badge = container.querySelector<HTMLElement>('[data-testid="log-unseen-badge"]');
  assert.ok(badge, '배지가 떠야 한다');
  assert.equal(badge!.textContent, '99+', '표시 텍스트는 99+ 로 축약');
  assert.equal(badge!.getAttribute('aria-label'), '새 로그 150건',
    'aria-label 은 스크린리더에 실제 수치를 그대로 전달');
});
