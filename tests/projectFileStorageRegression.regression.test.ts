...
test('CAT-B-4 (갭): MediaAssetStore 인터페이스는 삭제 API(remove/deleteById) 를 제공하지 않는다 — 단일 파일 삭제 UX 는 상위 레이어가 담당', () => {
  ...
  // 속성 접근 수준에서 삭제 계열 API 가 없음을 확인. Object.keys 는 런타임 구현에 따라
  // 혼란을 줄 수 있어 직접 접근으로 검증한다.
  const probe = store as unknown as Record<string, unknown>;
  assert.equal(typeof probe.remove, 'undefined', 'remove 가 인터페이스에 없어야 한다');
  assert.equal(typeof probe.deleteById, 'undefined', 'deleteById 가 인터페이스에 없어야 한다');
  assert.equal(typeof probe.deleteByProject, 'undefined', 'deleteByProject 가 인터페이스에 없어야 한다');
  // 저장소 소스에 삭제 계열 키워드가 아예 없음을 정적 검증으로 추가 고정
  const src = readFileSync(STORE_PATH, 'utf8');
  assert.doesNotMatch(src, /remove\(|deleteById|deleteByProject/);
