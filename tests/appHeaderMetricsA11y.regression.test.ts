...
test('6개 메트릭 칩 모두 role="status" 를 쓴다(zoom~collaboration)', () => {
  // 헤더 블록 안의 role="status" 개수를 세서 메트릭 6개 + 오프라인 배너 1개 = 7 인지
  // 확인한다. `<header>` ~ `</header>` 사이만 잘라 카운트해야 다른 토큰 배지와 혼동하지
  // 않는다. 오프라인 배너(`data-testid="offline-banner"`)도 헤더 안에 있어 함께 잡히며,
  // 메트릭 칩과 분리해 카운트하면 회귀 시 오발사를 줄인다.
  const headerMatch = SRC.match(/<header[\s\S]*?<\/header>/);
  assert.ok(headerMatch, 'header 블록을 찾지 못했다');
  const headerSrc = headerMatch![0];
  const statusRoles = headerSrc.match(/role="status"/g) ?? [];
  // 메트릭 칩 6개는 data-testid="header-metric-*" 로만 식별해 카운트한다.
  const metricChips = headerSrc.match(/data-testid="header-metric-[a-z]+"/g) ?? [];
  assert.equal(metricChips.length, 6, `헤더 메트릭 칩 수는 6 이어야 한다(실제 ${metricChips.length})`);
  assert.ok(
    statusRoles.length >= 6,
    `헤더 role="status" 수는 메트릭 6개 이상이어야 한다(실제 ${statusRoles.length})`,
  );
});
