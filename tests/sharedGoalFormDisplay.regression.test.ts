...
test('P1 회귀 — selectedProjectId 가 빈 문자열 "" 이어도 null 과 동일하게 폼 렌더 금지', () => {
  // ProjectManagement 스위칭 도중 일시적으로 '' 가 흘러가는 순간을 시뮬레이터가
  // null 과 동일 취급하는지 고정. 주석(ProjectManagement.tsx:101) 이 말한 회귀를
  // 판정기 레벨에서도 동일하게 잠근다.
  const outcome = evaluateSharedGoalFormVisibility({
    ...BASE_INPUT,
    selectedProjectId: '',
...
test('P1 회귀 — activeGoal.status 가 미지의 값이어도 active 아닌 한 create 모드(방어적 폴백)', () => {
  // 서버 스키마 확장 중 임시로 흘러든 미지의 status(pending 같은)는 편집
  // 대상이 아니므로 create 로 수렴시킨다. isActive 판정이 엄격한 equality 여야
  // 한다는 계약.
  const unknown = { ...ACTIVE_GOAL, status: 'pending' as unknown as SharedGoal['status'] };
  const outcome = evaluateSharedGoalFormVisibility({
    ...BASE_INPUT,
    activeGoal: unknown,
  });
  assert.equal(outcome.mode, 'create');
  assert.equal(outcome.prefilledFrom, null);
});
