...
// TODO(prompts.ts): buildLeaderPlanPrompt 에 sharedGoal 필드가 정식 도입되면
// 아래 test 를 활성화해 "프롬프트 본문 단일 출처" 계약을 잠근다. 지금은
// 시뮬레이터가 goalBlock 을 선두에 덧붙이는 방식이라, prompts.ts 본체에
// sharedGoal 을 추가한 PR 에서 이 테스트를 통과시키면 된다.
test.skip('S2 — [미구현] buildLeaderPlanPrompt 가 sharedGoal 을 네이티브 필드로 받아 프롬프트 본문에 삽입한다', () => {
  // const prompt = buildLeaderPlanPrompt({
  //   agent: LEADER, project: PROJECT, peers: MEMBERS,
  //   trigger: 'auto-dev', sharedGoal: GOAL_V1,
  // });
  // assert.match(prompt, /\[공동 목표\]/);
  // assert.ok(prompt.includes(GOAL_V1.text));
});
