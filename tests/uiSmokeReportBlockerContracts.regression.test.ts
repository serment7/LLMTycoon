...
test('CONTRACT §1.2 · App.tsx 가 `claude-session:status` 소켓을 구독해야 한다', () => {
  assert.match(
    APP_TSX,
    /newSocket\.on\(['"]claude-session:status['"][\s\S]{0,400}setSessionStatus/,
...
test('CONTRACT §2.1 · claude-usage:reset 핸들러가 sessionStatus 를 "active" 로 복원해야 한다', () => {
  // hydrate 는 누적을 0 으로만 리셋하고 sessionStatus 는 건드리지 않으므로,
  // reset 이벤트 핸들러가 명시적으로 setSessionStatus('active') 를 호출해야 한다.
  assert.match(
    APP_TSX,
    /newSocket\.on\(['"]claude-usage:reset['"][\s\S]{0,500}setSessionStatus\(\s*['"]active['"]\s*\)/,
...
test('CONTRACT §2.4 · ProjectManagement "PR 대상 선택" 버튼이 managed 가 비어 있을 때 disabled 로 닫혀야 한다', () => {
...
// ─── CONTRACT §2.4 보조 · 비활성 상태에서의 사유 안내 문구 회귀 잠금 ───────────────

test('CONTRACT §2.4 보조 · disabled 상태 title 에 "연동을 먼저 추가하세요" 안내가 포함된다', () => {
  // #de4bd2fc — 버튼이 눌리지 않는 이유를 툴팁/스크린리더에 노출해야 "왜 꺼져 있지?"
  // 라는 2차 혼란을 막는다. 안내 문구가 사라지면 해결되지 않은 결함으로 간주한다.
  assert.match(
    PROJECT_MGMT,
    /setShowPrTargetSelector\(true\)[\s\S]{0,1200}연동을 먼저 추가하세요/,
    '§2.4 — 비활성 사유가 title/aria 에 노출되지 않으면 사용자가 "버튼이 왜 꺼져 있는지" 를 알 수 없다'
  );
  assert.match(
    PROJECT_MGMT,
    /setShowPrTargetSelector\(true\)[\s\S]{0,1400}aria-disabled=/,
    'aria-disabled 를 함께 노출해 스크린리더 사용자도 비활성 상태를 인지하도록 한다'
  );
});
