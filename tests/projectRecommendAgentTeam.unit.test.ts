...
test('R4. heuristicTeam — 키워드에 따라 Designer/QA/Researcher 가 추가된다', () => {
  const basic = heuristicTeam('간단한 CLI 유틸');
  assert.deepEqual(basic.map((r) => r.role), ['Leader', 'Developer']);

  const design = heuristicTeam('UI 디자인 개편 + 화면 리워크');
  assert.ok(design.some((r) => r.role === 'Designer'));

  const full = heuristicTeam('보안 테스트 회귀 + 시장 조사 + UI 개편');
  const roles = full.map((r) => r.role);
